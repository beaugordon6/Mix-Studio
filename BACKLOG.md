...
- Acceptance:
  - [x] Fifty randomized fake-Comfy outage runs lose and duplicate zero jobs and preserve order, cancellation, and profile isolation.
...
- Verification: First reliability-spine milestone verified 2026-09-02: focused runtime identity, workflow contract, offline queue, Element staging, dependency, compatibility, cancellation, and recovery tests passed 112/112. A read-only live attestation identified canonical ComfyUI 0.34.0 as the configured source/models/input installation, and the production Character Element graph passed its versioned contract against the live `/object_info` without submitting work. Milestone 1's exact staged patch passed `node --check server.js`, `node --check public/app.js`, and the clean full `node --test` suite (1,306 passed, 0 failed, 1 skipped). Milestone 2's stable-ID reconciliation, journal migration, and operation-journal tests passed 29/29 focused; its exact staged patch passed syntax checks and the clean full suite (1,326 passed, 0 failed, 1 skipped) in an isolated checkout. Milestone 3's durable-cancellation, queue-recovery, fake-Comfy, gallery-finalization, and wiring tests passed focused checks; its exact staged patch passed both syntax checks and the clean full suite (1,360 passed, 0 failed) in an isolated checkout. Milestone 4's output validation, durable staging, journal, gallery-adapter, crash-recovery, and production-wiring tests passed 33/33 focused; its exact staged patch passed `node --check server.js`, `node --check public/app.js`, and the clean full `node --test` suite (1,389 passed, 0 failed) in an isolated checkout. Milestone 5's exact isolated release candidate passed 146/146 focused availability, port-drift, foreign-runtime, offline-upload, queue-reorder, cancellation, deletion, and upscale fault tests, then the clean full `node --test` suite (1,528 passed, 0 failed); both syntax checks passed. One preceding full-suite run exposed the existing timing-sensitive Xet progress test and the immediate identical rerun passed. Milestone 6's 50-seed real-HTTP fake-Comfy outage matrix passed with zero lost or duplicate jobs while preserving FIFO order, cancellations, and profile isolation. Incident-ledger, runtime-identity, supervisor, correlation-header, and owner support-bundle tests passed 38/38; the exact clean isolated candidate then passed `node --check server.js`, `node --check public/app.js`, and the full `node --test` suite (1,537 passed, 0 failed). Current and previous Character/Location/Prop capability snapshots plus foreign-install, wrong-model, and incompatible-node guards passed focused checks; each bad fixture returned a specific typed 409 and made zero submission calls.
- Notes: Created from the 2026-09-02 cross-task incident review. Recurring clusters were installation/port/input-namespace drift, pre-submit durability gaps, dependency success checked against the wrong installation, toast-only failures, shared-worktree deployment contamination, cloud capacity ambiguity, and insufficient behavioral fault tests. Provider capacity counts as blocking when Mix allows an attempt that cannot run. Milestone 1 adds stable install and per-process instance fingerprints, rejects a foreign source/input/models runtime before every Comfy write, keys the node-schema cache to the live instance, adds exact Character and Location/Prop Element contracts, includes shared CLIP/VAE readiness and Krea core gating, and keeps all profile-owned durable jobs visible when Comfy is offline. Milestone 2 journals ordinary Create/Edit/Elements and Strength Hunt intents before Element staging or `/prompt`, uses one caller-supplied canonical UUID end to end, and requires authoritative absence from both Comfy queue and history before a same-ID retry; Comfy does not deduplicate duplicate IDs. Milestone 3 persists cancellation tombstones before Comfy I/O, reconciles targeted queued/running cancellation after reconnect, suppresses gallery completion after cancellation, keeps the queue truthful while offline, and adds a reusable stateful fake Comfy endpoint plus a pure idempotent gallery-finalization planner. Milestone 4 wires deterministic, crash-replayable gallery finalization into ordinary manual Create, Edit, Character Element, Location Element, and Prop Element jobs: completed image bytes are signature-validated, fsync-staged, atomically catalogued with stable item/history identities, protected from hard reset, and never resubmitted after output readiness. Cross-task evidence ranks runtime identity/availability drift as a top-tier incident driver: both a foreign Comfy installation occupying the expected address and a canonical Comfy process crash have surfaced as the same port-refused symptom. Milestone 5 finishes the first availability and data-safety repair: Mix verifies the canonical installation before following it to a changed port, reconnects without a Mix restart, starts only the configured source checkout, blocks duplicate launch attempts, stops repeated crash loops, and reconciles preserved work after recovery. Uploaded/reference/Element inputs are retained by Mix and restaged after outages; queue reorder, cancellation, standalone/post upscales, and input deletion now use crash-replayable receipts. Milestone 6 makes failures measurable: every uncaught request error receives a correlation ID and privacy-safe typed ledger entry, Comfy recovery/attention transitions are recorded, and the owner can export a content-free support bundle. A cross-workflow audit found the remaining dominant durability gap: production still uses a three-kind job journal while the generic operation journal is not wired into edit sequences, Smart children, video, image composite, face repair, or cloud H3. Those workflow families remain P0 and must not be claimed reliable until their production crash tests pass. The first reusable edit-sequence/Smart child coordinator is implemented and passed 28/28 focused tests for immutable intent, dependency order, ambiguous-submit reconciliation, cancellation, restart recovery, finalization, and profile isolation; production wiring and real process-crash tests are still required. A cross-process maintenance lease now passes 4/4 focused tests for mutual exclusion, stale-process recovery, owner-only release, and corrupt-lock fail-closed behavior; route integration is still required. Element capability snapshots now cover the current and previous supported schemas, but ordinary Create/Edit, upscale, Director/video, and composite workflows still lack versioned pre-submit contracts. The generic-error inventory prioritizes typed body/JSON errors first, followed by missing assets/dependencies and builder/schema failures across those routes. Browser journeys remain in progress; WebKit currently lacks its Playwright runtime on this host.
